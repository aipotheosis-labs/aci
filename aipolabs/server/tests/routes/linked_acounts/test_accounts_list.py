...
def test_list_linked_accounts_no_filters(
    ...
    dummy_linked_account_apikey_github_project_1: LinkedAccount,
) -> None:
...
def test_list_linked_accounts_filter_by_account_name(
    ...
    response = test_client.get(
        ...
        headers={"x-api-key": dummy_api_key_1},
        params={
            "linked_account_owner_id": dummy_linked_account_apikey_github_project_1.linked_account_owner_id
        },
    )
    assert response.status_code == status.HTTP_200_OK, response.json()
    assert (
    ...
    assert response.json()[0]["app_id"] == str(dummy_linked_account_apikey_github_project_1.app_id)
    assert (
        response.json()[0]["linked_account_owner_id"]
        == dummy_linked_account_apikey_github_project_1.linked_account_owner_id
    )

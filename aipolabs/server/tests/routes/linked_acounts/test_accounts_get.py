...
def test_get_linked_account(
    ...
    ENDPOINT_1 = (
        f"{config.ROUTER_PREFIX_LINKED_ACCOUNTS}/" f"{dummy_linked_account_google_project_1.id}"
    )
    ENDPOINT_2 = (
        f"{config.ROUTER_PREFIX_LINKED_ACCOUNTS}/" f"{dummy_linked_account_google_project_2.id}"
    )
    ...
    assert (
        ...
        == dummy_linked_account_google_project_1.id
    )
    ...
    assert (
        LinkedAccountPublic.model_validate(response.json()).id
        == dummy_linked_account_google_project_2.id
    )
